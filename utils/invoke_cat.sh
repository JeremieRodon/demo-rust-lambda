...
usage () {
    ...
    echo ""
    echo "Repeatedly call GET <API_URL>/cat?m=<m>&n=<n> with m=3 and n=8 unless overritten"
    echo ""
    ...
    echo -e "-m <integer>\tThe 'm' number for the Ackermann algorithm. (Default: 3)"
    echo -e "-n <integer>\tThe 'n' number for the Ackermann algorithm. (Default: 8)"
    echo ""
    echo "OPTIONS:"
...
seq 1 $COUNT | xargs -Iunused -P$PARALLEL_TASKS curl -s --retry 5 --retry-connrefused "$API_URL/cat?m=$ARG_M&n=$ARG_N" > /dev/null
